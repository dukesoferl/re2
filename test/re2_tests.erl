...
replace_test() ->
  ...
  ?assertEqual({error,replace}, re2:replace("hello world","k+","L")),
  ...
  ?assertEqual({error,replace}, re2:replace("hello world",RegExR1,"L")),
...
match_test() ->
  ...
  ?assertMatch({'EXIT',{badarg,_}}, (catch re2:match("hello", RegExA, [ok]))),
  ?assertMatch({'EXIT',{badarg,_}}, (catch re2:match("hello", "h.*o", [ok]))),
  ?assertMatch({'EXIT',{badarg,_}}, (catch re2:match("hello", "h(?<name.*o"))),

  ?assertEqual(match, re2:match("heLlo", ".*ello", [caseless,{capture,none}])),
  ?assertMatch({'EXIT',{badarg,_}},
               (catch re2:match("Hello", RegExA, [caseless]))),

  ?assertEqual({match,[<<>>,<<>>,<<>>]},
               re2:match(<<"hello">>, RegExA, [{capture,['A',2,"B"],binary}])),
  ...
  ?assertEqual({match,[<<"hello">>,<<"h">>,<<"ell">>]},
               re2:match(<<"hello">>, <<"(h)(.*)o">>, [{capture,all,binary}])),
